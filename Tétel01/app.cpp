...
private:
	...
	public:
		Csomopont(const Csomopont& from) :betu(from.betu), balNulla(from.balNulla?from.balNulla:0), jobbEgy(from.jobbEgy?from.jobbEgy:0)	//másoló konstruktor
		{}
	private:
		Csomopont & operator= (const Csomopont &);
		//SAJÁT
	public:
		bool isCurrent(const LZWBinFa& lzwfa) const
		{
			if (lzwfa.fa == this)
				return true;
			return false;
	...
	//SAJÁT
public:
	LZWBinFa(const LZWBinFa& from) : gyoker(from.gyoker)
	{
		this->setCurrent(this->gyoker, from.gyoker, from);		//setCurrent rekurzivitás miatt kell és egyszerűsít
	}
	void setCurrent(Csomopont& cst, const Csomopont& csf, const LZWBinFa& f)		//keressük a 'fa' elemet
	{
		if (csf.isCurrent(f))
			this->fa = &cst;
		else
		{															//párhuzamos bejárás preorder módon
			setCurrent(*cst.nullasGyermek(), *csf.nullasGyermek(), f);		
			setCurrent(*cst.egyesGyermek(), *csf.egyesGyermek(), f);
		}
	}
	//~SAJÁT
private:
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	for (char c : s)
		binfa1 << c;
	LZWBinFa binfa2(binfa1);
	cout << binfa1 << endl << "-----------------------------\n" << binfa2 << endl;
	for (char c : "00000000000")
	{
		binfa1 << c;
		binfa2 << c;
	}
